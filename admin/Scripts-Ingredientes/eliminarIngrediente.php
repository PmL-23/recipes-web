...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['id']) && isset($_SESSION['nomUsuario'])) {
        ...
        if (!Permisos::tienePermiso('Eliminar Ingrediente', $usuarioID)) {
            echo json_encode(['success' => false, 'message' => 'Error, no posee el permiso para eliminar ingredientes.']);
            exit();
        }
        
    }else{
        echo json_encode(['success' => false, 'message' => 'Necesitas iniciar sesión para poder eliminar ingredientes..', 'id_publicacion_receta' => $id_publicacion_receta]);
        exit();
    }
