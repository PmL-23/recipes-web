...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['id']) && isset($_SESSION['nomUsuario'])) {
        ...
        if (!Permisos::tienePermiso('Crear Categoria', $usuarioID)) {
            echo json_encode(['success' => false, 'message' => 'Error, no posee el permiso para crear categorias.']);
            exit();
        }
        
    }else{
        echo json_encode(['success' => false, 'message' => 'Necesitas iniciar sesión para poder crear categorias..', 'id_publicacion_receta' => $id_publicacion_receta]);
        exit();
    }
