...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['id']) && isset($_SESSION['nomUsuario'])) {
        ...
        if (!Permisos::tienePermiso('Editar Etiqueta', $usuarioID)) {
            echo json_encode(['success' => false, 'message' => 'Error, no posee el permiso para editar etiquetas.']);
            exit();
        }
        
    }else{
        echo json_encode(['success' => false, 'message' => 'Necesitas iniciar sesión para poder editar etiquetas..', 'id_publicacion_receta' => $id_publicacion_receta]);
        exit();
    }
