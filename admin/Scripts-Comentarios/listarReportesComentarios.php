...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if (isset($_SESSION['id']) && isset($_SESSION['nomUsuario'])) {
        ...
        if (!Permisos::tienePermiso('Listar Reportes Comentario', $usuarioID)) {
            echo json_encode(['success' => false, 'message' => 'Error, no posee el permiso para listar comentarios reportados.']);
            exit();
        }
        
    }else{
        echo json_encode(['success' => false, 'message' => 'Necesitas iniciar sesión para poder listar comentarios reportados..', 'id_publicacion_receta' => $id_publicacion_receta]);
        exit();
    }
